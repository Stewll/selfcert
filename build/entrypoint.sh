...
echo ""
echo "Root Key Password: $ROOT_KEY_PASS" && echo $ROOT_KEY_PASS > Results/Pass/rootCA.key.pass
echo "CSR Key Password: $CSR_KEY_PASS" && echo $CSR_KEY_PASS > Results/Pass/csr.key.pass
echo " " 
echo "Creating a PFX file for Keepass containing the root key and the root certificate:"
openssl pkcs12 -export -out Results/pfx_file_for_keepass/dynamics.pfx -inkey Results/Keys/rootCA.key -in Results/CAcerts/root.pem -passin pass:$ROOT_KEY_PASS -passout pass:$ROOT_KEY_PASS

# Clean up
rm -rf Results/Keys Results/CAcerts Results/certs Results/CSR Results/Pass  crt.conf certs.tar.gz Results/certs.tar.gz.enc Results/pfx_file_for_keepass 
# Exit
exit 0
